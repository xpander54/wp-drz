...
(function($) {
	...
	droneOptions = {
		...
		guid: function() {
			return String(new Date().getTime()) + String(Math.round(Math.random()*8999+1000));
		},
		...
		attach: function(options) {
			...
			$('.drone-option-image-select:not(.drone-ready)', options).addClass('drone-ready').each(function() {
				...
				current.click(function(e) {
					$('.drone-option-image-select', _options).filter(function() { return this != _this; }).find('.options').hide();
			...
			$('.drone-option-conditional-tags:not(.drone-ready)', options).addClass('drone-ready').each(function() {
				...
				$('> .conditions', this).on('blur', '> li > label select', function() {
					...
					$(this).hide().prev('span').text($(':selected', this).text()).show();
